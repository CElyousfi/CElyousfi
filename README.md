<!-- HEADER SECTION -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=000000&height=300&section=header&text=Charaf%20El%20Yousfi&fontSize=90&animation=fadeIn&fontAlignY=38&desc=Self-challenging%20AI%20Engineer%20from%20Casa%20to%20the%20World%20🗽&descAlignY=51&descAlign=62&fontColor=ffffff" width="100%"/>
</div>

<!-- ANIMATED GIF SECTION -->
<div align="center">
  <img src="https://i.imgur.com/kJx43Ei.gif" width="500px" alt="8-bit Developer Vibing"/>
</div>

<!-- SOCIAL BADGES -->
<div align="center">
  <a href="https://charafportfolio-charafs-projects-3e6e28fc.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-%23000000.svg?style=for-the-badge&logo=firefox&logoColor=white" alt="Portfolio" />
  </a>
  <a href="https://www.linkedin.com/in/charaf-el-yousfi/">
    <img src="https://img.shields.io/badge/LinkedIn-%23000000.svg?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://medium.com/@charafelyousfi3">
    <img src="https://img.shields.io/badge/Medium-%23000000.svg?style=for-the-badge&logo=medium&logoColor=white" alt="Medium" />
  </a>
  <a href="mailto:charafelyousfi3@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-%23000000.svg?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
  </a>
</div>

<!-- ANIMATED TYPING -->
<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Inter&size=22&duration=4000&pause=1000&color=FFFFFF&center=true&vCenter=true&width=600&height=100&lines=AI+Engineering+%7C+Data+Science;Machine+Learning+%7C+Deep+Learning;Always+questioning+assumptions" />
</div>

<br>

<!-- ABOUT ME SECTION -->
<h2 align="center">About Me</h2>

<p align="center">
  I am a relentless perfectionist who thrives at the intersection of AI, data science, and mathematical rigor. My approach is defined by critical thinking and self-questioning — constantly challenging my own assumptions to achieve true innovation.
</p>
<p align="center">
  As an Engineering Student at EMSI Casablanca, I've cultivated expertise in machine learning, deep learning, and data analytics while developing practical solutions that push technological boundaries.
</p>
<p align="center">
  My passion lies in creating AI systems that make complex information accessible and solve real-world problems with mathematical precision.
</p>

<!-- TECH STACK SECTION -->
<h2 align="center">Tech Stack</h2>

<table align="center" border="0" cellspacing="0" cellpadding="0" style="border: none;">
  <tr style="border: none;">
    <td style="border: none;">
      <h3 align="center">Languages</h3>
      <div align="center">
        <img src="https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=white" />
        <img src="https://img.shields.io/badge/JavaScript-000000?style=for-the-badge&logo=javascript&logoColor=white" />
        <img src="https://img.shields.io/badge/C%23-000000?style=for-the-badge&logo=c-sharp&logoColor=white" />
      </div>
    </td>
  </tr>
  <tr style="border: none;">
    <td style="border: none;">
      <h3 align="center">AI & ML</h3>
      <div align="center">
        <img src="https://img.shields.io/badge/TensorFlow-000000?style=for-the-badge&logo=tensorflow&logoColor=white" />
        <img src="https://img.shields.io/badge/PyTorch-000000?style=for-the-badge&logo=pytorch&logoColor=white" />
        <img src="https://img.shields.io/badge/scikit--learn-000000?style=for-the-badge&logo=scikit-learn&logoColor=white" />
        <img src="https://img.shields.io/badge/Pandas-000000?style=for-the-badge&logo=pandas&logoColor=white" />
        <img src="https://img.shields.io/badge/NumPy-000000?style=for-the-badge&logo=numpy&logoColor=white" />
      </div>
    </td>
  </tr>
  <tr style="border: none;">
    <td style="border: none;">
      <h3 align="center">Tools & Technologies</h3>
      <div align="center">
        <img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white" />
        <img src="https://img.shields.io/badge/Docker-000000?style=for-the-badge&logo=docker&logoColor=white" />
        <img src="https://img.shields.io/badge/MySQL-000000?style=for-the-badge&logo=mysql&logoColor=white" />
        <img src="https://img.shields.io/badge/Linux-000000?style=for-the-badge&logo=linux&logoColor=white" />
      </div>
    </td>
  </tr>
</table>

<!-- FLAGSHIP PROJECTS SECTION -->
<h2 align="center">Flagship Projects</h2>

<table align="center" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="50%">
      <h3 align="center">SummaRise 🦊</h3>
      <p align="center">
        <a href="https://github.com/CElyousfi/SummaRise" target="_blank">
          <img src="https://cdn.dribbble.com/users/1162077/screenshots/5403918/media/d5dccb5fc779e53a352b9a8bdfe4270e.gif" width="100%" alt="SummaRise"/>
        </a>
      </p>
      <p align="center"><strong>NLP, JavaScript, Machine Learning</strong> - A gamified browser extension with Finn the fox that analyzes and simplifies complex Terms of Service & Privacy Policies.</p>
    </td>
    <td width="50%">
      <h3 align="center">Medical Decision Support 🏥</h3>
      <p align="center">
        <a href="#" target="_blank">
          <img src="https://cdn.dribbble.com/users/1681442/screenshots/4735886/media/29246aa34fdeef51c8dc6eaeb7a69b2d.gif" width="100%" alt="Medical App"/>
        </a>
      </p>
      <p align="center"><strong>Flutter, CNN, OCR, Hybrid ML Models</strong> - A cutting-edge mobile application integrated with advanced AI services to enhance medical diagnosis.</p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3 align="center">Document Analysis System 📝</h3>
      <p align="center">
        <a href="#" target="_blank">
          <img src="https://cdn.dribbble.com/users/2514208/screenshots/9802960/media/153b271d393ac0ac47ea1dd90a06f6d5.gif" width="100%" alt="Document Analysis"/>
        </a>
      </p>
      <p align="center"><strong>OCR, FAISS, Python</strong> - Intelligent system that extracts, analyzes, and organizes medical document data through AI-driven document understanding.</p>
    </td>
    <td width="50%">
      <h3 align="center">AI Financial Advisor 💰</h3>
      <p align="center">
        <a href="#" target="_blank">
          <img src="https://cdn.dribbble.com/users/2514208/screenshots/9457622/media/2860978e513e84b13ad26e05146635fe.gif" width="100%" alt="Financial Advisor"/>
        </a>
      </p>
      <p align="center"><strong>NLP, Python, Data Analysis</strong> - Intelligent financial assistant leveraging NLP to analyze spending patterns and provide personalized recommendations.</p>
    </td>
  </tr>
</table>

<!-- WORKLOAD & METRICS SECTION -->
<h2 align="center">Analytics & Development Focus</h2>

<div align="center">
  <table border="0" cellspacing="0" cellpadding="0" style="width: 100%; border-collapse: separate; border-spacing: 10px;">
    <tr>
      <td width="33%" align="center" style="background-color: #111111; border-radius: 10px; padding: 20px;">
        <h3>AI Development</h3>
        <div style="font-size: 36px; margin: 10px 0; font-weight: bold;">70%</div>
        <p style="margin: 5px 0; color: #aaa;">Python | TensorFlow | PyTorch</p>
        <div style="height: 10px; background-color: #333; border-radius: 5px; margin: 15px 0;">
          <div style="height: 100%; width: 70%; background-color: #ffffff; border-radius: 5px;"></div>
        </div>
      </td>
      <td width="33%" align="center" style="background-color: #111111; border-radius: 10px; padding: 20px;">
        <h3>Web Development</h3>
        <div style="font-size: 36px; margin: 10px 0; font-weight: bold;">20%</div>
        <p style="margin: 5px 0; color: #aaa;">JavaScript | React | Flask</p>
        <div style="height: 10px; background-color: #333; border-radius: 5px; margin: 15px 0;">
          <div style="height: 100%; width: 20%; background-color: #ffffff; border-radius: 5px;"></div>
        </div>
      </td>
      <td width="33%" align="center" style="background-color: #111111; border-radius: 10px; padding: 20px;">
        <h3>DevOps & Others</h3>
        <div style="font-size: 36px; margin: 10px 0; font-weight: bold;">10%</div>
        <p style="margin: 5px 0; color: #aaa;">Docker | SQL | C#</p>
        <div style="height: 10px; background-color: #333; border-radius: 5px; margin: 15px 0;">
          <div style="height: 100%; width: 10%; background-color: #ffffff; border-radius: 5px;"></div>
        </div>
      </td>
    </tr>
  </table>
</div>

<!-- TECH JOURNEY SECTION -->
<h2 align="center">My Tech Journey</h2>

<table align="center" border="0" cellspacing="10" cellpadding="10">
  <tr>
    <td width="50%" style="background-color: #111111; border-radius: 10px; padding: 15px;">
      <h3>2025 🚀</h3>
      <p>Expected Engineering Degree from EMSI, Casablanca</p>
    </td>
    <td width="50%" style="background-color: #111111; border-radius: 10px; padding: 15px;">
      <h3>2024 🏆</h3>
      <p>Oracle Database Administration I Certification</p>
    </td>
  </tr>
  <tr>
    <td width="50%" style="background-color: #111111; border-radius: 10px; padding: 15px;">
      <h3>2024 💻</h3>
      <p>Internship at Ministry of Interior - Application Development</p>
    </td>
    <td width="50%" style="background-color: #111111; border-radius: 10px; padding: 15px;">
      <h3>2024 📊</h3>
      <p>Marketing Analytics Specialization from Meta</p>
    </td>
  </tr>
  <tr>
    <td width="50%" style="background-color: #111111; border-radius: 10px; padding: 15px;">
      <h3>2023 🖥️</h3>
      <p>Internship at OMIADATA Group - Custom CRM/ERP Module</p>
    </td>
    <td width="50%" style="background-color: #111111; border-radius: 10px; padding: 15px;">
      <h3>2023 🐍</h3>
      <p>Python for Everybody Specialization from University of Michigan</p>
    </td>
  </tr>
  <tr>
    <td colspan="2" style="background-color: #111111; border-radius: 10px; padding: 15px; text-align: center;">
      <h3>2022 🎓</h3>
      <p>Started Engineering Degree at EMSI, Casablanca</p>
    </td>
  </tr>
</table>

<!-- KEY METRICS SECTION -->
<h2 align="center">Key Metrics</h2>

<div align="center">
  <table border="0" cellspacing="10" cellpadding="10" style="border-collapse: separate; border-spacing: 10px;">
    <tr>
      <td align="center" style="background-color: #111111; border-radius: 10px; padding: 15px;">
        <h3>+210</h3>
        <p>Total Projects</p>
      </td>
      <td align="center" style="background-color: #111111; border-radius: 10px; padding: 15px;">
        <h3>+125</h3>
        <p>Clients Worldwide</p>
      </td>
      <td align="center" style="background-color: #111111; border-radius: 10px; padding: 15px;">
        <h3>24</h3>
        <p>Years Old</p>
      </td>
      <td align="center" style="background-color: #111111; border-radius: 10px; padding: 15px;">
        <h3>500K+</h3>
        <p>Lines of Code</p>
      </td>
    </tr>
  </table>
</div>

<!-- CURRENT FOCUS SECTION -->
<h2 align="center">Current Focus</h2>

<div align="center">
  <table border="0" cellspacing="0" cellpadding="0" style="background-color: #111111; border-radius: 10px; width: 80%; margin: 0 auto;">
    <tr>
      <td style="padding: 10px;">• Advanced Deep Learning Architectures</td>
      <td style="padding: 10px;">• Generative AI & Large Language Models</td>
    </tr>
    <tr>
      <td style="padding: 10px;">• Computer Vision in Medical Applications</td>
      <td style="padding: 10px;">• MLOps & Production ML Systems</td>
    </tr>
    <tr>
      <td style="padding: 10px;">• Reinforcement Learning</td>
      <td style="padding: 10px;">• AI Ethics & Responsible AI</td>
    </tr>
  </table>
</div>

<!-- STATS SECTION -->
<h2 align="center">GitHub Stats</h2>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=CElyousfi&show_icons=true&count_private=true&hide=issues&theme=dark&bg_color=000000&hide_border=true&icon_color=ffffff&title_color=ffffff&text_color=ffffff" alt="GitHub Stats" />
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=CElyousfi&theme=dark&background=000000&ring=ffffff&fire=ffffff&currStreakLabel=ffffff&hide_border=true" alt="GitHub Streak" />
</div>

<!-- MEDIUM ARTICLES SECTION -->
<h2 align="center">Latest Articles</h2>

<div align="center">
  <a href="https://medium.com/@charafelyousfi3">
    <img src="https://github-readme-medium-recent-article.vercel.app/medium/@charafelyousfi3/0" width="80%" alt="Recent Article" />
  </a>
</div>

<!-- FOOTER -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=000000&height=120&section=footer&animation=fadeIn" width="100%"/>
</div>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=CElyousfi&style=for-the-badge&color=000000" alt="Profile Views" />
</div>

<!-- Visitor counter -->
<img src="https://hit.yhype.me/github/profile?user_id=CElyousfi" width="0" height="0">
